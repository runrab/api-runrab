...
//setTimeout(better_sqlite3,10000)
//const row1 = db.prepare('SELECT * FROM img WHERE id=100').get();//得到一个
//console.log(row1.url);



/**
 //better_sqlite3()实现内存模式
 function better_sqlite3(){
    const fs = require('fs');
    const path = require("path");
    const file = path.resolve('../api_runrab.db');;  //这里写的就是数据库文件的路径
    const Database = require('better-sqlite3');
